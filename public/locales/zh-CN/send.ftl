...
# SendCrew is a brand name and should not be localized.
downloadMessage = 您的朋友使用 SendCrew 向您发送一个文件。该服务允许用户以安全、私密、受加密的链接分享一个文件，链接到期后文件将从网上彻底抹除。
...
# SendCrew is a brand name and should not be localized.
notSupportedDetail = 很可惜，您的浏览器不支持 SendCrew 所使用的 Web 技术。请改用其他浏览器。我们推荐使用 Firefox！
notSupportedLink = 为什么不支持我的浏览器？
notSupportedOutdatedDetail = 很可惜，此版本的 Firefox 不支持 SendCrew 所使用的 Web 技术。您需要更新浏览器才能使用它。
...
nevermindButton = 没关系
legalHeader = 条款和隐私
legalNoticeTestPilot = SendCrew 目前是一个 Test Pilot 实验，并遵守 Test Pilot <a>服务条款</a>和<a>隐私声明</a>。您可以在<a>这里</a>了解此实验及数据收集的有关信息。
legalNoticeMozilla = 使用 SendCrew 网站亦受到 Mozilla <a>网站隐私声明</a>和<a>网站使用条款</a>的约束。
...
javascriptRequired = SendCrew 需要 JavaScript
whyJavascript = 为什么 SendCrew 需要 JavaScript？
